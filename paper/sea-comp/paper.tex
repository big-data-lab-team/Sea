...
\begin{document}
\title{Sea: A lightweight data-management library for Big Data scientific computing}
...
\begin{abstract}
    The recent influx of scientific data has lead to scientific
computing transitioning from compute intensive to data intensive. Whereas many Big Data
...
applications executing on HPC clusters without the need to reinstrument workflows.
Sea leverages GNU C library interception to intercept
...
\IEEEPARstart{E}{fficient} data-placement
strategies have become essential in minimizing the costs of processing Big Data.
Strategies, such as data locality and in-memory computing, improve application runtime by ensuring
data access occur on the most efficient available storage. Existing Big Data frameworks, such as
MapReduce~\cite{dean2008mapreduce}, Apache Spark~\cite{zaharia2016apache} and
Dask~\cite{rocklin2015dask}, improve runtime performance through the incorporation of these data-placement
strategies.
...
scientific tools typically lack data-placement mechanisms. In contrast, efforts have
been placed on efficient storage representation, such as HDF5~\cite{hdf5} and Zarr~\cite{zarr}, and
the sharing of data, through initiatives like DataLad~\cite{wagner2022fairly}.

Data-placement strategies can complement existing solutions by selecting where data should be accessed at runtime,
facilitating the transfer and processing of large datasets across different infrastructures, and placing the different
file formats at the preferred storage layer to maximize efficiency. However, converting adapting existing scientific frameworks
and applications to Big Data frameworks requires significant effort, necessitating the rewrite of many well established applications.
Furthermore, as noted in \cite{mehta2017comparative}, Big Data frameworks are still difficult to use for the processing of certain
scientific data, such as imaging data.
...
Our goal is to provide a data-placement add-on to standardized scientific computing applications executing on High Performance Computing (HPC)
clusters. The tools of particular interest to us consist of various tasks that communicate to each other via a POSIX-compliant file system. Whereas
they can be platform-agnostic, users typically leverage them within an HPC context, particularly when processing large volumes of data, due to their
inherent reliance on POSIX-compliance.

We developed Sea, a user-space data-placement library, that provides efficient data placement without reinstrumentation of the underlying application.
Given a predefined storage hierarchy, Sea can prefetch, cache, flush and evict data to and from a permanent storage location. To evaluate Sea,
we have designed a performance model, as well as evaluate its performance on a synthetic Big Data application running on a representative neuroimaging dataset.
...
data-placement C++ library for applications running on HPC systems. Its main

...
\title{Ephemeral hierarchical data-management for neuroimaging applications \TG{Update title as discussed previously}}
...
\begin{abstract}
  ...
  associated with large data transfers \TG{use a word better than transfers}. A major challenge in adapting neuroimaging
...
%\noindent Please note: Abbreviations should be introduced at the first mention in the main text – no abbreviations lists or tables should be included. Structure of the main text is provided below.
  ...
  \section{Introduction}\label{sec:sea_neuro:introduction}
    ...
    transfer-related overheads. \TG{add a few sentences to summarize the main features of Sea} We view Sea as a complement
...
\end{figure*}
    ...
    HPC \TG{It'd be useful to add a sentence before explaining that neuroimaging applications typically run on HPC systems} compute-local resources are only accessible during the reserved
...
\TG{I think the methods need to be summarized in 2-3 sentences here}
\TG{I would also say "The SPM pipeline" rather than "SPM" (same for all pipelines)}

Speedups are obtained by using Sea when Lustre
performance has been degraded by busy writers (Figure~\ref{fig:seaneuro:slashbin}), although performance is variable. On average, a single SPM
...
greater the speedup. We do occasionally observe slowdowns from using Sea. These slowdowns
...
\end{figure*}
  ...
  we can still obtain important speedups by using Sea. It is very likely that when we
  ...
  Our results with flushing enabled confirm this theory. When running with
  ...
  3.2$\times$, processing 8 ds001545 images.
    ...
    Sea really shines when it is executed on data-intensive pipelines. This
    ...
    Our results expose the complexities of performance predictions on HPC clusters. We present results on a dedicated cluster with which we have
    ...
    Each tool was set to only run the preprocessing pipeline. Scripts used to
    execute each pipeline can be found
    at~\url{github.com/valhayot/sea-slurm-scripts}. \TG{I'd group some of these short paragraphs together}
  ...
  \end{table*}
    ...
    alongside a Spark application that continuously reads and writes approximately
    ...
    read through a memmap (i.e. only loading necessary portions of the file to memory) with Lustre. Flushing would normally be necessary with

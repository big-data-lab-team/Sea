...
\title{Automated neuroimaging data-management with Sea}
...
%\noindent Please note: Abbreviations should be introduced at the first mention in the main text – no abbreviations lists or tables should be included. Structure of the main text is provided below.
  ...
  \section{Introduction}\label{sec:sea_neuro:introduction}
    ...
    rewrite standardized neuroimaging toolboxes for the
    frameworks. 
    ...
    %This is because even with large datasets, neuroimaging data processing
    %remains split between compute and data intensive components.
    ...
    In this paper, we evaluate Sea (Chapter~\ref{chapter:sea-comp}), a data-management library for HPC, on three neuroimaging preprocessing pipelines
...
\end{figure*}
    ...
    As neuroimaging pipeline results are typically required post-processing and
    ...
    \caption{Pipeline execution characteristics based on the processing of a single fMRI image using a single application process}\label{table:seaneuro:pipelines}
    \end{table*}
...
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{figures/beluga_sea_tmpfs.pdf}%

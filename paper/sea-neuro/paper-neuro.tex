...
\begin{abstract}
  ...
  where its benefits are limited. Even in scenarios with minimal performance gain, Sea
  can improve overall filesystem performance by limiting the number of files on shared filesystems.
\end{abstract}
\begin{document}
...
%\noindent Please note: Abbreviations should be introduced at the first mention in the main text – no abbreviations lists or tables should be included. Structure of the main text is provided below.
  ...
  \section{Introduction}\label{sec:sea_neuro:introduction}
    ...
    To address the community's data management needs, many tools have been developped.
    ...
    amount to very large datasets. Differences in the number and
    ...
    cost-effective solution to accessing a wide array of hardware resources for
    ...
    of the shared file system for all active users. Solutions for
    ...
    In this paper, we evaluate Sea~\cite{hayot2022sea}, a data-management library designed to reduce
    transfer-related overheads in user space, on real neuroimaging applications and quatify its
    benefits to neuroimaging research. We view Sea as a complement
...
\end{figure*}
    ...
    aimed at slower storage devices to faster devices whenever possible.
    ...
    processing with data management operations, this is accomplished via a 
    separate thread (known as the
    ``flusher'') that moves data from the caches to long-term storage.
    Users must inform Sea of files
  ...
  Interfacing with Sea is achieved using a user-specified mountpoint. The mountpoint

...
lib_opts=-ldl -lm -lpthread -lstdc++ -liniparser
debug_opts=-g -ggdb
...
build/sea.so: build/passthrough.o build/functions.o build/logger.o build/config.o build/sea.o
	gcc ${debug_opts} -shared build/passthrough.o build/functions.o build/logger.o build/config.o build/sea.o -o build/sea.so ${lib_opts}
build/mirror: build/config.o build/logger.o build/passthrough.o
	gcc ${debug_opts} src/mirrordirs.cpp build/config.o build/logger.o build/passthrough.o -o build/mirror ${lib_opts}
test/test_custom: tests/test_custom.cpp build/passthrough.o build/sea.o build/config.o build/logger.o
	gcc ${debug_opts} -std=c++14 -Wall -fPIC tests/test_custom.cpp build/passthrough.o build/sea.o build/config.o build/logger.o -o tests/test_custom -lgtest ${lib_opts}
clean:
